...
# ---------------------------------------------------------------------------
# nvfp4_ds_mla 416-byte layout constants (GLM-5.2 sparse-MLA / DSA on sm_121a).
# ---------------------------------------------------------------------------
# Per-token record (uint8, 416 bytes):
#   bytes [  0:256]  : 512 x E2M1 (FP4) packed 2/byte (the kv_lora_rank=512
#                       NoPE latent). LOW nibble = even index, HIGH = odd.
#   bytes [256:288]  : 32  x float8_e4m3 block scales (one per 16 NoPE elems);
#                       scale = amax(block)/6 (FP4 max magnitude is 6.0).
#   bytes [288:416]  : 64  x bfloat16 RoPE (UNCHANGED from fp8_ds_mla, relocated).
# Dequant: nope[i] = E2M1_LUT[nibble(i)] * float(bsf[i // 16])   (global-scale GS
# fixed at 1.0, so the GS field is omitted; the per-16 fp8 BSF carry magnitude).
# Byte-exact with flashinfer nvfp4_kv_quantize/nvfp4_kv_dequantize (verified:
# manual-vs-flashinfer dequant max-abs == 0.0, rel-err ~0.093 on N(0,4) input).
_NVFP4DS_NOPE_DIM = 512        # number of E2M1 NoPE values
_NVFP4DS_ROPE_DIM = 64         # number of bf16 RoPE values (unchanged)
_NVFP4DS_BLK = 16              # NoPE elems per fp8 block scale
_NVFP4DS_NUM_SCALES = _NVFP4DS_NOPE_DIM // _NVFP4DS_BLK   # 32
_NVFP4DS_FP4_BYTES = _NVFP4DS_NOPE_DIM // 2               # 256
_NVFP4DS_SCALE_OFF = _NVFP4DS_FP4_BYTES                   # 256
_NVFP4DS_ROPE_OFF = _NVFP4DS_SCALE_OFF + _NVFP4DS_NUM_SCALES  # 288
_NVFP4DS_ENTRY_BYTES = _NVFP4DS_ROPE_OFF + 2 * _NVFP4DS_ROPE_DIM  # 416
_NVFP4DS_FP4_AMAX = 6.0        # max representable |E2M1| value

_NVFP4_LUT_CACHE: dict = {}


def _nvfp4_lut(device: "torch.device") -> "torch.Tensor":
    """Signed E2M1 lookup table: nibble (0..15) -> float value."""
    g = _NVFP4_LUT_CACHE.get(device)
    if g is None:
        g = torch.tensor(
...
# ===========================================================================
# FUSED gather-dequant + online-softmax for the nvfp4_ds_mla 416-B cache.
# Identical online-softmax recurrence to the fp8 fused kernel; only the NoPE
# dequant block differs (E2M1 nibble-unpack * per-16 fp8 BSF instead of
# fp8 * per-128 fp32 scale). RoPE block identical except rope_byte_off=288.
# ===========================================================================
@triton.jit
def _fused_gather_dequant_attn_nvfp4ds_kernel(
    q_ptr,                # bf16 [T, H, D]
    cache_ptr,            # uint8 [total_slots, 416]
    indices_ptr,          # int32 [T, K] global slot ids
    lens_ptr,             # int32 [T]
    lut_ptr,              # f32   [16] E2M1 LUT
    ...
    nope_dim: tl.constexpr,        # 512
    rope_dim: tl.constexpr,        # 64
    blk: tl.constexpr,             # 16 (elems per fp8 BSF)
    scale_off: tl.constexpr,       # 256
    rope_byte_off: tl.constexpr,   # 288
    num_candidates,
    scale: tl.constexpr,
    HEAD_BLOCK: tl.constexpr,
    BLOCK_N: tl.constexpr,
    ...
    nope_dmask = nope_off < nope_dim
    rope_dmask = rope_off < rope_dim
    # E2M1 byte/nibble layout for each NoPE elem.
    byte_idx = nope_off // 2
    is_hi = (nope_off % 2) == 1
    sc_idx = nope_off // blk                        # 0..31 per-16 fp8 scale index
    ...
    for n0 in range(0, num_candidates, BLOCK_N):
        ...
        slot = tl.load(
            indices_ptr + token_idx * stride_idx_t + cand * stride_idx_k,
            mask=cand_in_range, other=-1,
        )
        cand_valid = (slot >= 0) & (slot < total_slots) & (cand < valid_len)
        base = cache_ptr + slot.to(tl.int64)[:, None] * stride_cache_s

        # ---- NoPE [BLOCK_N, 512]: E2M1 nibble * per-16 fp8 block scale ----
        nmask = cand_valid[:, None] & nope_dmask[None, :]
        packed = tl.load(base + byte_idx[None, :], mask=nmask, other=0).to(tl.int32)
        nib = tl.where(is_hi[None, :], (packed >> 4) & 0xF, packed & 0xF)
        e2m1 = tl.load(lut_ptr + nib, mask=nmask, other=0.0)
        bsf = tl.load(base + scale_off + sc_idx[None, :], mask=nmask, other=0)
        scale_f32 = bsf.to(tl.float8e4nv, bitcast=True).to(tl.float32)
        kv_nope = tl.where(nmask, e2m1 * scale_f32, 0.0).to(tl.bfloat16)  # [BLOCK_N,512]

        # ---- RoPE [BLOCK_N, 64]: bf16 at rope_byte_off=288 ----
        rmask = cand_valid[:, None] & rope_dmask[None, :]
        rope_ptr = (base + rope_byte_off).to(tl.pointer_type(tl.bfloat16))
        kv_rope = tl.load(rope_ptr + rope_off[None, :], mask=rmask, other=0.0)
    ...
    state_base = token_idx * stride_state_t
    tl.store(max_score_ptr + state_base + head_offsets * stride_state_h,
             running_max, mask=head_mask)
    tl.store(denom_ptr + state_base + head_offsets * stride_state_h,
             running_denom, mask=head_mask)
    tl.store(
        acc_ptr + token_idx * stride_acc_t
        + head_offsets[:, None] * stride_acc_h + nope_off[None, :] * stride_acc_d,
        running_acc, mask=head_mask[:, None] & nope_dmask[None, :],
    )


def _fused_gather_dequant_attend_nvfp4ds(
    q, cache_flat, indices, lens, scale, max_score, denom, acc, *,
    block_n: int = _FUSED_BLOCK_N, head_block: int | None = None,
    num_warps: int = _FUSED_NUM_WARPS, num_stages: int = _FUSED_NUM_STAGES,
) -> None:
    """Fused gather+dequant+attend over the nvfp4_ds_mla (416-B) paged cache."""
    T, H, D = q.shape
    K = indices.shape[1]
    total_slots = cache_flat.shape[0]
    assert cache_flat.dtype == torch.uint8
    assert cache_flat.shape[1] == _NVFP4DS_ENTRY_BYTES
    assert D == _DQK, f"fused path expects head_dim {_DQK}, got {D}"
    if head_block is None:
        head_block = _FUSED_HEAD_BLOCK if H >= 16 else next_power_of_2(max(H, 1))
        head_block = max(head_block, 16)
    grid = (T, triton.cdiv(H, head_block))
    _fused_gather_dequant_attn_nvfp4ds_kernel[grid](
        q, cache_flat, indices, lens, _nvfp4_lut(q.device),
        max_score, denom, acc,
        q.stride(0), q.stride(1), q.stride(2),
        cache_flat.stride(0), indices.stride(0), indices.stride(1),
        max_score.stride(0), max_score.stride(1),
        acc.stride(0), acc.stride(1), acc.stride(2),
        total_slots, H,
        _NVFP4DS_NOPE_DIM, _NVFP4DS_ROPE_DIM, _NVFP4DS_BLK,
        _NVFP4DS_SCALE_OFF, _NVFP4DS_ROPE_OFF,
        K, float(scale),
        HEAD_BLOCK=head_block, BLOCK_N=block_n,
        BLOCK_NOPE=_NVFP4DS_NOPE_DIM, BLOCK_ROPE=_NVFP4DS_ROPE_DIM,
...
def flash_mla_with_kvcache_triton(
    ...
    assert is_fp8_kvcache, "Triton decode path requires is_fp8_kvcache=True (fp8_ds_mla / nvfp4_ds_mla)"
    ...
    kc = k_cache
    assert kc.dtype == torch.uint8, "ds_mla decode cache must be uint8"
    # Route by the per-token record width: 416 B == nvfp4_ds_mla, 656 B == fp8.
    _entry_bytes = kc.shape[-1]
    _is_nvfp4 = (_entry_bytes == _NVFP4DS_ENTRY_BYTES)
    cache_flat = kc.reshape(-1, _entry_bytes)
    ...
    if fused_enabled:
        ...
        if _is_nvfp4:
            _fused_gather_dequant_attend_nvfp4ds(
                q_flat, cache_flat, idx2d, lens,
                float(softmax_scale), max_score, denom, acc_v,
            )
        else:
        ...
        if _is_nvfp4:
            kv_gathered, valid = _gather_dequant_nvfp4ds(cache_flat, idx2d)
        else:
...
# ===========================================================================
# nvfp4_ds_mla STORE + gather-dequant (GLM-5.2 sparse-MLA, sm_121a)
# ===========================================================================
# The STORE path prefers flashinfer ``nvfp4_kv_quantize`` (the ground-truth FP4
# packer: it emits [M,256] uint8 packed E2M1 + [M,32] uint8 fp8 BSF, exactly the
# 416-B record's NoPE region). The packed bytes are written to the per-slot
# record with a capturable Triton scatter (slot<0 skipped in-kernel). The RoPE
# 64xbf16 are copied verbatim to ROPE_OFF=288.


@triton.jit
def _nvfp4ds_scatter_kernel(
    fp4_ptr,        # uint8 [N, 256] packed E2M1 NoPE
    bsf_ptr,        # uint8 [N, 32]  fp8 block scales
    rope_ptr,       # bf16  [N, 64]  RoPE
    slot_ptr,       # int32 [N]      destination global slot ids (-1 skip)
    cache_ptr,      # uint8 [total_slots, ENTRY_BYTES]
    stride_fp4_n,
    stride_bsf_n,
    stride_rope_n,
    stride_cache_s,
    fp4_bytes: tl.constexpr,     # 256
    num_scales: tl.constexpr,    # 32
    scale_off: tl.constexpr,     # 256
    rope_off: tl.constexpr,      # 288
    rope_dim: tl.constexpr,      # 64
    BLK_FP4: tl.constexpr,       # >=256
    BLK_SC: tl.constexpr,        # >=32
    BLK_RP: tl.constexpr,        # >=64
):
    n = tl.program_id(0).to(tl.int64)
    slot = tl.load(slot_ptr + n)
    if slot < 0:
        return
    dst = cache_ptr + slot.to(tl.int64) * stride_cache_s

    o4 = tl.arange(0, BLK_FP4)
    m4 = o4 < fp4_bytes
    fp4 = tl.load(fp4_ptr + n * stride_fp4_n + o4, mask=m4, other=0)
    tl.store(dst + o4, fp4, mask=m4)

    osc = tl.arange(0, BLK_SC)
    msc = osc < num_scales
    bsf = tl.load(bsf_ptr + n * stride_bsf_n + osc, mask=msc, other=0)
    tl.store(dst + scale_off + osc, bsf, mask=msc)

    orp = tl.arange(0, BLK_RP)
    mrp = orp < rope_dim
    rope = tl.load(rope_ptr + n * stride_rope_n + orp, mask=mrp, other=0.0)
    rope_dst = (dst + rope_off).to(tl.pointer_type(tl.bfloat16))
    tl.store(rope_dst + orp, rope, mask=mrp)


def _nvfp4ds_store(
    kv_c_normed: torch.Tensor,   # bf16 [N, 512] NoPE latent (kv_lora_rank)
    k_pe: torch.Tensor,          # bf16 [N, 64]  RoPE
    cache_flat: torch.Tensor,    # uint8 [total_slots, 416]
    slot_mapping: torch.Tensor,  # int32/int64 [N] global slot ids (-1 skip)
) -> None:
    """Quantize the NoPE latent to nvfp4_ds_mla and write the 416-B records.

    Uses flashinfer ``nvfp4_kv_quantize`` (global-scale GS=1.0) as the FP4
    packer, then a capturable Triton scatter to place [E2M1 | fp8 BSF | bf16
    RoPE] at each slot. Mirrors MiMo's ``_nvfp4_store_diffkv``.
    """
    ...
    assert kv_c_normed.shape[1] == _NVFP4DS_NOPE_DIM
    assert k_pe.shape[1] == _NVFP4DS_ROPE_DIM
    device = cache_flat.device

    gs = torch.ones(1, device=device, dtype=torch.float32)
    fp4, bsf = nvfp4_kv_quantize(
        kv_c_normed.reshape(N, _NVFP4DS_NOPE_DIM).contiguous().to(torch.bfloat16),
        gs,
    )  # fp4: [N,256] uint8, bsf: [N,32] uint8
    rope = k_pe.reshape(N, _NVFP4DS_ROPE_DIM).contiguous().to(torch.bfloat16)
    slot = slot_mapping.contiguous()

    _nvfp4ds_scatter_kernel[(N,)](
        fp4, bsf, rope, slot, cache_flat,
        fp4.stride(0), bsf.stride(0), rope.stride(0), cache_flat.stride(0),
        _NVFP4DS_FP4_BYTES,
        _NVFP4DS_NUM_SCALES,
        _NVFP4DS_SCALE_OFF,
        _NVFP4DS_ROPE_OFF,
        _NVFP4DS_ROPE_DIM,
        BLK_FP4=next_power_of_2(_NVFP4DS_FP4_BYTES),
        BLK_SC=next_power_of_2(_NVFP4DS_NUM_SCALES),
        BLK_RP=next_power_of_2(_NVFP4DS_ROPE_DIM),
        num_warps=4,
    )


# ---------------------------------------------------------------------------
# nvfp4_ds_mla gather-dequant kernel (materialize path; mirror of fp8 variant).
# Dequant: nope[i] = E2M1_LUT[nibble(i)] * float8(bsf[i//16]); RoPE = bf16 verbatim.
# ---------------------------------------------------------------------------
@triton.jit
def _gather_dequant_nvfp4ds_kernel(
    cache_ptr,            # uint8 [total_slots, 416]
    indices_ptr,          # int32 [T, K] global slot ids, -1 invalid
    lut_ptr,              # f32   [16] E2M1 LUT
    out_ptr,              # bf16  [T, K, 576] gathered dequantized kv
    ...
    nope_dim: tl.constexpr,       # 512
    rope_dim: tl.constexpr,       # 64
    blk: tl.constexpr,            # 16 (elems per scale)
    scale_off: tl.constexpr,      # 256
    rope_byte_off: tl.constexpr,  # 288
    BLOCK_D: tl.constexpr,        # >= 576
):
    t = tl.program_id(0).to(tl.int64)
    k = tl.program_id(1).to(tl.int64)
    ...
    # ---- NoPE: E2M1 nibble * per-16 fp8 block scale ----
    # Two NoPE elems per packed byte: byte = dim//2, low nibble = even index.
    byte_idx = dim_offsets // 2
    is_hi = (dim_offsets % 2) == 1
    packed = tl.load(base + byte_idx, mask=nope_mask & is_valid, other=0).to(tl.int32)
    nib = tl.where(is_hi, (packed >> 4) & 0xF, packed & 0xF)
    e2m1 = tl.load(lut_ptr + nib, mask=nope_mask & is_valid, other=0.0)
    # block scale: fp8_e4m3 at scale_off + dim//16
    sc_idx = dim_offsets // blk
    bsf = tl.load(base + scale_off + sc_idx, mask=nope_mask & is_valid, other=0)
    scale_f32 = bsf.to(tl.float8e4nv, bitcast=True).to(tl.float32)
    nope = e2m1 * scale_f32

    # ---- RoPE: bf16 values at rope_byte_off ----
    rope_elem = tl.maximum(dim_offsets - nope_dim, 0)
    rope_ptr = (base + rope_byte_off).to(tl.pointer_type(tl.bfloat16))
    rope = tl.load(rope_ptr + rope_elem, mask=rope_mask & is_valid, other=0.0).to(
        tl.float32
    )
...
def _gather_dequant_nvfp4ds(
    cache_flat: torch.Tensor,   # uint8 [total_slots, 416]
    indices: torch.Tensor,      # int32 [T, K] global slot ids
) -> tuple[torch.Tensor, torch.Tensor]:
    """Gather + dequantize nvfp4_ds_mla cache rows. Returns (kv [T,K,576] bf16,
    valid [T,K] bool). Invalid rows are zero-filled."""
    assert cache_flat.dtype == torch.uint8
    assert cache_flat.dim() == 2 and cache_flat.shape[1] == _NVFP4DS_ENTRY_BYTES, (
        f"expected nvfp4_ds_mla cache [total_slots, {_NVFP4DS_ENTRY_BYTES}], "
        f"got {tuple(cache_flat.shape)}"
    )
    assert indices.dim() == 2
    T, K = indices.shape
    ...
    idx_c = indices.contiguous()
    valid = (idx_c >= 0) & (idx_c < total_slots)
    block_d = max(64, next_power_of_2(_DQK))
    grid = (T, K)
    _gather_dequant_nvfp4ds_kernel[grid](
        cache_flat,
        idx_c,
        _nvfp4_lut(device),
        out,
